...
def run_model(df_zones, w_fire, w_flood, alpha, small_budget, q_min):
    ...
    for j, row in enumerate(zones):
        ...
        # Условие "Старая сирена" логически скрыто, оставляем для совместимости
...
df_z = load_data()[0] if load_data() else None
...
if df_z is not None:
    ...
    if st.sidebar.button("🚀 ЗАПУСТИТЬ ОПТИМИЗАЦИЮ", type="primary"):
        ...
        st.pydeck_chart(pdk.Deck(
            layers=[
                pdk.Layer("GeoJsonLayer", boundary_data, opacity=0.3, stroked=True, filled=True,
                          get_fill_color=[100, 150, 200, 20], get_line_color=[100, 100, 100, 150],
                          line_width_min_pixels=1),
                pdk.Layer("ScatterplotLayer", data=df_res, get_position=["Долгота", "Широта"], get_color="color",
                          get_radius=1500, pickable=True, filled=True)
            ],
            initial_view_state=pdk.ViewState(latitude=df_res['Широта'].mean(), longitude=df_res['Долгота'].mean(),
                                             zoom=6),
            tooltip={"html": "<b>{Н.П.}</b><br/>{Тип угрозы}<br/>{ТСО}"}
        ))
        st.dataframe(df_res)